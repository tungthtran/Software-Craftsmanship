...
public final class ActivityGroup {
    ...
    ActivityGroup() {
        this.timePointsGroup = new HashSet<>();
    }
    ...
    private static Long dependencyTimeLine(Dependency dependency, Map<TimePoint, Long> timeLineMap) {
    ...
    private static Long maxTimeLine(TimePoint timePoint, Map<TimePoint, Long> timeLineMap) {
        return timePoint.getDependencies().stream().mapToLong(dependency -> dependencyTimeLine(dependency, timeLineMap))
                                                    .max().orElse(0);
    }
